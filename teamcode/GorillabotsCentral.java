...
public abstract class GorillabotsCentral extends LinearOpMode {
   ...
   // public Grabber grabber;
    ...
    public void runNewTrack0(){
        ...
        sleep(100);
        gyro.resetAngle();
        MoveUntilRangeFbothAc(17,130,.5,0);
        TurnAbsolute(-90,.29,.45);
        gyro.resetAngle();
        dropWobble();
        grabber.setPosition(Grabber_Open);
        sleep(400);
        MoveUntilEncoderGYRO(8,270,.4,0);//29
        MoveUntilRangeFbothAc(17,180,.4,0);
        sleep(100);
        MoveUntilEncoderGYRO(9.4,90,.4,0);
        sleep(300);
        grabber.setPosition(Grabber_Close);
        sleep(400);
        MoveUntilEncoderGYRO(60,0,.85,0);
        sleep(200);
        grabber.setPosition(Grabber_Open);
        sleep(50);
        MoveUntilEncoderGYRO(6,270,.8,0);
    }
    public void runNewTrack1(){
        Servo grabber;
        grabber = hardwareMap.get(Servo.class, "grabber");
        final double Grabber_Close = .6;
        final double Grabber_Open = .05;
        ...
        stopIntake();
        TurnAbsolute(0,.3,.6);
        MoveUntilRangeFbothAc(15,180,.65,0);
        sleep(200);
        ...
        MoveUntilEncoderGYRO(12,0,.7,0);
        stopMotors();

    }

    public void runNewTrack4(){
        Servo grabber;
        grabber = hardwareMap.get(Servo.class, "grabber");
        final double Grabber_Close = .6;
        final double Grabber_Open = .05;
        ...
        MoveUntilEncoderGYRO(5,225,1,0);
        MoveUntilRangeRbothAc(24.5,270,.5,0);
        robot.Guard.setPosition(RobotHardware.guardDown);
        sleep(200);
        startIntake();
        moveTransfer();
        rampShooter(-.15);
        MoveUntilEncoderGYRO(56,0,.5,0);
        sleep(400);
        reverseTransfer();
        stopShooter();
        sleep(400);
        ...
        stopIntake();
        TurnAbsolute(0,.3,.6);
        MoveUntilRangeFbothAc(14,180,.65,0);
        sleep(150);
        MoveUntilRangeRbothAc(6,270,.5,0);
        TurnAbsolute(-88,.3,.6);
        dropWobble();
        sleep(700);
        grabber.setPosition(Grabber_Open);
        MoveUntilEncoderGYRO(26,270,.7,-88);
    }
   ...
   // public void MoveUntilRangeB(double distance, double direction, double power) {
    ...
    public void MoveUntilRangeRG(double distance, double direction, double power, double gyroT) {
        ...
        setMotorsBackwards();
        MoveTo(direction, power);
        while (abs(sensors.getDistanceR() - distance) > 1 && opModeIsActive()) {
            if(gamepad1.a){
                distance = 0;
            }
            MoveTowR(direction, power, (gyro.getAngle() - gyroT) / 50);
    ...
    public void MoveUntilRangeL(double distance, double direction, double power) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceL() - distance > 2 && opModeIsActive()) {
            MoveTo(direction, power);
    ...
    public void MoveUntilRangeLboth(double distance, double direction, double power) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceL() - distance > .5 && opModeIsActive()) {
            MoveTo(direction, power);
            telemetry.addData("d", sensors.getDistanceL());
            telemetry.update();
        }
    ...
    public void MoveUntilRangeRboth(double distance, double direction, double power) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceR() - distance > .5 && opModeIsActive()) {
            MoveTo(direction, power);
            telemetry.addData("d", sensors.getDistanceR());
            telemetry.update();
        }
    ...
    public void MoveUntilRangeLbothAc(double distance, double direction, double power, double gyroT) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceL() - distance > .5 && opModeIsActive()) {
       ...
       /* sleep(550);
        while (sensors.getDistanceL() - distance > .5 && opModeIsActive()) {
            MoveTowR(direction, power*.7, (gyro.getAngle() - gyroT) / 50);
        }
        while (sensors.getDistanceL()-distance <.5 && opModeIsActive()){
            MoveTowR(direction, -power*.7, (gyro.getAngle() - gyroT) / 50);
        }
    ...
    public void MoveUntilRangeFbothAc(double distance, double direction, double power, double gyroT) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceF() - distance > .5 && opModeIsActive()) {
            MoveTowR(direction, power, (gyro.getAngle() - gyroT) / 50);
        }
        while (sensors.getDistanceF()-distance <.5 && opModeIsActive()){
            MoveTowR(direction, -power, (gyro.getAngle() - gyroT) / 50);
    ...
    public void MoveUntilRangeRbothAc(double distance, double direction, double power, double gyroT) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceR() - distance > .5 && opModeIsActive()) {
    ...
    public void MoveUntilRangeFboth(double distance, double direction, double power) {
        setDriveEncoderOn(false);
        setMotorsBackwards();
        //MoveTo(direction, power);
        while (sensors.getDistanceF() - distance > .5 && opModeIsActive()) {
    ...
    public void TurnAbsolute(double TargetDegree, double min, double max) {
        ...
        while (abs(angleDiff) > .5 && runtime.seconds() < maxTime && opModeIsActive()) {
            double leftPower;
            double rightPower;
